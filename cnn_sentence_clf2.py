...
def deep_CNN(input_shape, label_class):
    ...
    model.add( Dropout(0.25, input_shape=(input_shape[0],input_shape[1],input_shape[2])) )
    #cov layers
    model.add(Convolution2D( nb_filter = 250, nb_row = 3, nb_col = 1,
    ...
    model.add(Flatten())
    model.add(Dense(output_dim = hidden_dims, activation='relu'))
    model.add(Dropout(0.25))

    #Fully Connected Layer as output layer
    #model.add(Dense(output_dim = label_class, activation='sigmoid'));
    model.add(Dense(label_class, activation='sigmoid'));
...
if __name__ == "__main__":
    ...
    print ("%s %s %s" %(input_shape[0], input_shape[1], input_shape[2]))
    print (label_class)
