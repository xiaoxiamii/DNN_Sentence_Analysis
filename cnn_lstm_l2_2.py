...
def DNN(data, input_shape, label_class):
    ...
    model.add(Convolution2D( nb_filter = 30 , nb_row = 3, nb_col = 1,
    ...
    model.add(Reshape(dims=(100 ,input_shape[1]-4)))
    model.add(LSTM(50,return_sequences=True,input_length=100))
    
    model.add(Flatten())
    model.add(Dropout(0.5))
    model.add(Dense(output_dim = 256, activation = 'relu', W_regularizer=l2(0.01) ))
    ...
    model.compile(loss='mean_absolute_error', class_mode = 'binary',optimizer=adadelta)
...
if __name__ == "__main__":
    ...
    all_num = 10
    valid_rate = 0.9
    path = './log/cnn_lstm_l2_in_log_1216'
